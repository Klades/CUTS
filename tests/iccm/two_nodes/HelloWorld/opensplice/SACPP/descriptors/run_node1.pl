...
sub run_node_daemons {
  for ($i = 0; $i < $daemons; ++$i)
  {
      ...
      $d_param = "-ORBEndpoint $iiop -s $node_app -n $nodename=$iorfile -t 30 --locality-config $CUTS_ROOT/bin/handlers/opensplice.locality.config";
...
$NS = new PerlACE::Process ("$TAO_ROOT/orbsvcs/Naming_Service/tao_cosnaming", "-m 0 -ORBEndpoint iiop://$hostname:60003 -o ns.ior");
$NS->Spawn ();
...
print "Node 1 ready, press enter after Node 2 has started";
$pause = <STDIN>;
