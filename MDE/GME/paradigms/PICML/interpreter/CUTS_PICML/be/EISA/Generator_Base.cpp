...
//
// CUTS_EISA_Generator_Base
//
CUTS_EISA_Generator_Base::CUTS_EISA_Generator_Base (void)
{
...
// include
//
std::string CUTS_EISA_Generator_Base::
...
write_TwowayOperation_begin (const PICML::TwowayOperation & twoway)
{
  ...
  if (!params.empty ())
  {
    ...
    std::for_each (iter,
                   params.end (),
                   boost::bind (&CUTS_EISA_Generator_Base::write_parameter,
                                this,
                                _1));
  }
  else
...
write_OperationBase_begin (const PICML::OperationBase & operation_base)
{
  ...
  if (!inparams.empty ())
  {
    ...
    std::for_each (iter,
                   inparams.end (),
                   boost::bind (&CUTS_EISA_Generator_Base::write_in_parameter,
                                this,
                                _1));
  }
  else
