...
generate_idlgen_project (const CUTS_BE_IDL_Node & node)
{
  ...
  this->ctx_.project_
    ...
    << "              -Wb,skel_export_include=" << name << "_skel_export.h \\" << std::endl;

  if (has_component)
  {
    this->ctx_.project_
      << "              -Wb,exec_export_macro=" << macro_basename << "_EXEC_Export \\" << std::endl
      << "              -Wb,exec_export_include=" << name << "_exec_export.h \\" << std::endl
  ...
  if (has_events)
  {
    // Insert the TCP/IP stub source files.
    this->stub_files_.insert ("TCPIP_" + name + "C.cpp");

    // Generate the OpenSplice project.
    this->ctx_.project_
      << "project (" << name << "_OpenSplice_IDL_Gen) : splice_ts_defaults, requires_opensplice {" << std::endl
      << "  custom_only = 1" << std::endl
      << std::endl
    ...
    // Insert the auto-generated files into the stub project.
...
generate_stub_project (const CUTS_BE_IDL_Node & node)
{
  ...
  // Generate the project.
  this->ctx_.project_
    << "project (" << stub_name << ") : avoids_ace_for_tao, cuts_chaos_ccm_stub, cuts_codegen_defaults {" << std::endl
...
generate_svnt_project (const CUTS_BE_IDL_Node & node)
{
  ...
  this->ctx_.project_
    << "project (" << svnt_project << ") : ciao_servant, cuts_chaos_ccm, cuts_codegen_defaults {" << std::endl
