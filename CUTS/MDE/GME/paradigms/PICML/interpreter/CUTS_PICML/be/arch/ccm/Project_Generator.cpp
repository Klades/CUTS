...
generate (const CUTS_BE_IDL_Node & node)
{
  ...
  bool has_interface = CUTS_BE::has_interface (node.file_);

  if (has_interface || has_component)
  {
...
generate_idlgen_project (const CUTS_BE_IDL_Node & node)
{
  ...
  this->ctx_.project_
    << (has_component || has_events ? "component" : "ciao") << "idldefaults, cuts_codegen_defaults {" << std::endl
  ...
  if (has_component || has_interface)
  {
    this->ctx_.project_
  ...
  if (has_component && !this->ctx_.traits_->generate_default_servant ())
  {
    this->ctx_.project_
      << std::endl
