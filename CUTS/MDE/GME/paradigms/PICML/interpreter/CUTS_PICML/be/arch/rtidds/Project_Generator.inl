// -*- C++ -*-
// $Id$

///////////////////////////////////////////////////////////////////////////////
// CUTS_BE_Project_File_Open_T

CUTS_INLINE
CUTS_BE_Project_File_Open_T <CUTS_BE_RTIDDS::Context, CUTS_BE_Impl_Node>::
CUTS_BE_Project_File_Open_T (CUTS_BE_RTIDDS::Context & context)
: base_type (context)
{

}

CUTS_INLINE
CUTS_BE_Project_File_Open_T <CUTS_BE_RTIDDS::Context, CUTS_BE_Impl_Node>::
~CUTS_BE_Project_File_Open_T (void)
{

}

///////////////////////////////////////////////////////////////////////////////
// CUTS_BE_Project_File_Begin_T

CUTS_INLINE
CUTS_BE_Project_File_Begin_T <CUTS_BE_RTIDDS::Context, CUTS_BE_Impl_Node>::
CUTS_BE_Project_File_Begin_T (CUTS_BE_RTIDDS::Context & context)
: base_type (context)
{

}

CUTS_INLINE
CUTS_BE_Project_File_Begin_T <CUTS_BE_RTIDDS::Context, CUTS_BE_Impl_Node>::
~CUTS_BE_Project_File_Begin_T (void)
{

}

///////////////////////////////////////////////////////////////////////////////
// CUTS_BE_Project_Write_T

CUTS_INLINE
CUTS_BE_Project_Write_T <CUTS_BE_RTIDDS::Context, CUTS_BE_Impl_Node>::
CUTS_BE_Project_Write_T (CUTS_BE_RTIDDS::Context & context)
: base_type (context)
{

}

CUTS_INLINE
CUTS_BE_Project_Write_T <CUTS_BE_RTIDDS::Context, CUTS_BE_Impl_Node>::
~CUTS_BE_Project_Write_T (void)
{

}

///////////////////////////////////////////////////////////////////////////////
// CUTS_BE_Project_File_End_T

CUTS_INLINE
CUTS_BE_Project_File_End_T <CUTS_BE_RTIDDS::Context, CUTS_BE_Impl_Node>::
CUTS_BE_Project_File_End_T (CUTS_BE_RTIDDS::Context & context)
: base_type (context)
{

}

CUTS_INLINE
CUTS_BE_Project_File_End_T <CUTS_BE_RTIDDS::Context, CUTS_BE_Impl_Node>::
~CUTS_BE_Project_File_End_T (void)
{

}

///////////////////////////////////////////////////////////////////////////////
// CUTS_BE_Project_File_Close_T

CUTS_INLINE
CUTS_BE_Project_File_Close_T <CUTS_BE_RTIDDS::Context, CUTS_BE_Impl_Node>::
CUTS_BE_Project_File_Close_T (CUTS_BE_RTIDDS::Context & context)
: base_type (context)
{

}

CUTS_INLINE
CUTS_BE_Project_File_Close_T <CUTS_BE_RTIDDS::Context, CUTS_BE_Impl_Node>::
~CUTS_BE_Project_File_Close_T (void)
{

}

///////////////////////////////////////////////////////////////////////////////
// CUTS_BE_Project_File_Open_T

CUTS_INLINE
CUTS_BE_Project_File_Open_T <CUTS_BE_RTIDDS::Context, CUTS_BE_IDL_Node>::
CUTS_BE_Project_File_Open_T (CUTS_BE_RTIDDS::Context & context)
: base_type (context)
{

}

CUTS_INLINE
CUTS_BE_Project_File_Open_T <CUTS_BE_RTIDDS::Context, CUTS_BE_IDL_Node>::
~CUTS_BE_Project_File_Open_T (void)
{

}

///////////////////////////////////////////////////////////////////////////////
// CUTS_BE_Project_File_Begin_T

CUTS_INLINE
CUTS_BE_Project_File_Begin_T <CUTS_BE_RTIDDS::Context, CUTS_BE_IDL_Node>::
CUTS_BE_Project_File_Begin_T (CUTS_BE_RTIDDS::Context & context)
: base_type (context)
{

}

CUTS_INLINE
CUTS_BE_Project_File_Begin_T <CUTS_BE_RTIDDS::Context, CUTS_BE_IDL_Node>::
~CUTS_BE_Project_File_Begin_T (void)
{

}

///////////////////////////////////////////////////////////////////////////////
// CUTS_BE_Project_Write_T

CUTS_INLINE
CUTS_BE_Project_Write_T <CUTS_BE_RTIDDS::Context, CUTS_BE_IDL_Node>::
CUTS_BE_Project_Write_T (CUTS_BE_RTIDDS::Context & context)
: base_type (context, false, "cuts_rtidds_ccm", "cuts_rtidds_ccm_stub")
{

}

CUTS_INLINE
CUTS_BE_Project_Write_T <CUTS_BE_RTIDDS::Context, CUTS_BE_IDL_Node>::
~CUTS_BE_Project_Write_T (void)
{

}

///////////////////////////////////////////////////////////////////////////////
// CUTS_BE_Project_File_End_T

CUTS_INLINE
CUTS_BE_Project_File_End_T <CUTS_BE_RTIDDS::Context, CUTS_BE_IDL_Node>::
CUTS_BE_Project_File_End_T (CUTS_BE_RTIDDS::Context & context)
: base_type (context)
{

}

CUTS_INLINE
CUTS_BE_Project_File_End_T <CUTS_BE_RTIDDS::Context, CUTS_BE_IDL_Node>::
~CUTS_BE_Project_File_End_T (void)
{

}

///////////////////////////////////////////////////////////////////////////////
// CUTS_BE_Project_File_Close_T

CUTS_INLINE
CUTS_BE_Project_File_Close_T <CUTS_BE_RTIDDS::Context, CUTS_BE_IDL_Node>::
CUTS_BE_Project_File_Close_T (CUTS_BE_RTIDDS::Context & context)
: base_type (context)
{

}

CUTS_INLINE
CUTS_BE_Project_File_Close_T <CUTS_BE_RTIDDS::Context, CUTS_BE_IDL_Node>::
~CUTS_BE_Project_File_Close_T (void)
{

}


