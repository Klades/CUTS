...
struct create_receptacle_t
{
  create_receptacle_t (GAME::Model & component)
    : component_ (component)
  {

  }

  void operator () (const GAME::Reference & facet) const
  {
    const std::string name (facet.name ());
    const GAME::FCO refobj (facet.refers_to ());
    ...
    if (GAME::create_if_not (this->component_, meta::RequiredRequestPort, receptacle,
...
generate (const GAME::Model & component)
{
  ...
  if (GAME::create_if_not (this->folder_, meta::File, driver_file,
      GAME::contains (boost::bind (std::equal_to <std::string> (),
                                   driver_filename,
  ...
  GAME::Model driver_component;

  if (GAME::create_if_not (package, meta::Component, driver_component,
      GAME::contains (boost::bind (std::equal_to <std::string> (),
                                   driver_filename,
                                   boost::bind (&GAME::Model::name, _1)))))
  {
    driver_component.name (driver_filename);
  }  
  ...
  std::for_each (facets.begin (),
                 facets.end (),
                 create_receptacle_t (driver_component));
...
get_file_info (const GAME::Model & component, fileinfo_t & info)
{
                             ...
                             GAME::Model & package)
{
  ...
  for (; iter != iter_end; ++ iter)
  {
    ...
    if (GAME::create_if_not (package, meta::Package, package,
        GAME::contains (boost::bind (std::equal_to <std::string> (),
                                     name,
                                     boost::bind (&GAME::Model::name, _1)))))
    {
