...
//
// CUTS_BE_DeploymentPlan_Begin_T
//
bool CUTS_BE_DeploymentPlan_Begin_T <CUTS_BE_Tioa>::
generate (const PICML::DeploymentPlan & plan)
{
  // Save the name of the deployment.
  std::string name = plan.name ();

  // Construct the filename of the TIOA deployment file.
  std::string filename =
    CUTS_BE_OPTIONS ()->output_directory_ + "/" + name + ".deployment.tioa";
  ...
  if (CUTS_BE_TIOA ()->outfile_.is_open ())
  {
    // Write the preamble to the deployment file.
    CUTS_BE_TIOA ()->outfile_
      << "%% -*- TIOA -*-" << std::endl
      << "%%" << std::endl
      << "%% @file " << name << ".deployment.tioa" << std::endl
      << "%%" << std::endl
      << "%% This file was generated by:" << std::endl
      << "%% $Id$" << std::endl
      << "%%" << std::endl
      << "%% Timed I/O Automata (TIOA) file for the " << name << " deployment. The" << std::endl
      << "%% component is designed to work within the CUTS' TIOA simulation" << std::endl
      << "%% framework. It is STRONGLY advised that you do not make in" << std::endl
      << "%% modifications to this file. If you do make modifications to" << std::endl
      << "%% this file, then you may obtain unknown results." << std::endl
...
//
// CUTS_BE_Deployment_Node_T
//
bool CUTS_BE_Deployment_Node_T <CUTS_BE_Tioa>::
generate (const PICML::Node & node)
{
  CUTS_BE_TIOA ()->outfile_
    << "    node_"  << node.name () << " : Host ("
    << node.uniqueId () << ", 100);" << std::endl;

  return true;
}

//
// CUTS_BE_Deployment_Location_T
//
bool CUTS_BE_Deployment_Location_T <CUTS_BE_Tioa>::
generate (const PICML::Component & component, const PICML::Node & node)
{
  std::string cname = component.name ();
  std::string nname = node.name ();

  CUTS_BE_TIOA ()->deployment_map_.
    insert (std::make_pair (component, node));

  return true;
}

//
// CUTS_BE_DeploymentPlan_End_T
//
bool CUTS_BE_DeploymentPlan_End_T <CUTS_BE_Tioa>::
generate (const PICML::DeploymentPlan & plan)
{
  if (!CUTS_BE_TIOA ()->deployment_map_.empty ())
  {
    // We are making the assumption all components in the <deployment_map_>
    // belong to the same assembly. Also, we are making the assumption we
    // are only deploying instances. We are not deploying shared components
    // or whole assemblies.

    PICML::MgaObject parent =
      CUTS_BE_TIOA ()->deployment_map_.begin ()->first.parent ();

    CUTS_BE_TIOA ()->outfile_
      << std::endl
      << "    %% system declaration" << std::endl
      << "    system : " << parent.name () << " (";

    // We need to generate the location of the first instance.
    CUTS_BE_Tioa::Deployment_Map::iterator iter =
      CUTS_BE_TIOA ()->deployment_map_.begin ();

    CUTS_BE_TIOA ()->outfile_ << iter->second.uniqueId ();

    // Now, generate the location of the remaining components.
    for (++ iter;
         iter != CUTS_BE_TIOA ()->deployment_map_.end ();
         iter ++)
    {
      CUTS_BE_TIOA ()->outfile_
        << ", " << iter->second.uniqueId ();
    }
  ...
  // Clear all the nodes in the mapping.
  CUTS_BE_TIOA ()->deployment_map_.clear ();
