...
void CUTS_BE_Project_File_Begin_T <CUTS_CHAOS::Context, CUTS_BE_Impl_Node>::
...
//
// CUTS_BE_Project_Write_T <CUTS_CHAOS::Context, CUTS_BE_Impl_Node>
//
void
CUTS_BE_Project_Write_T <CUTS_CHAOS::Context, CUTS_BE_Impl_Node>::
generate (const CUTS_BE_Impl_Node & node)
...
generate_exec_project (const CUTS_BE_Impl_Node & node)
{
  ...
  this->ctx_.project_
    << "project (" << project_name << ") : ciao_executor, cuts_opensplice, cuts_codegen_defaults {" << std::endl
...
generate_svnt_project (const CUTS_BE_Impl_Node & node)
{
  ...
  this->ctx_.project_
    << "project (" << svnt_project << ") : cuts_opensplice_ccm, cuts_codegen_defaults {" << std::endl
  ...
  this->ctx_.project_
    // Generate the source files
    << "  Source_Files {" << std::endl
    << "    CHAOS_" << container_name << "_svnt.cpp" << std::endl
    << "  }" << std::endl
    << std::endl
    // Generate the header files
    << "  Header_Files {" << std::endl
    << "    CHAOS_" << container_name << "_svnt.h" << std::endl
    << "  }" << std::endl
    << std::endl
    << "  Inline_Files {" << std::endl
    << std::endl
...
//
void
CUTS_BE_Project_File_End_T <CUTS_CHAOS::Context, CUTS_BE_Impl_Node>::
generate (const CUTS_BE_Impl_Node & node)
{
  this->ctx_.project_
...
//
void
CUTS_BE_Project_File_Close_T <CUTS_CHAOS::Context, CUTS_BE_Impl_Node>::
generate (void)
{
...
void CUTS_BE_Project_File_Begin_T <CUTS_CHAOS::Context, CUTS_BE_IDL_Node>::
...
//
void
CUTS_BE_Project_Write_T <CUTS_CHAOS::Context, CUTS_BE_IDL_Node>::
generate (const CUTS_BE_IDL_Node & node)
{
...
generate_ddsidlgen_project (const CUTS_BE_IDL_Node & node)
{
  ...
  // Generate the project.
  this->ctx_.project_
    << "project (" << node.name_ << "_DDS_IDL_Gen) : splice_ts_defaults, requires_opensplice {" << std::endl
    << "  custom_only = 1" << std::endl
    << std::endl
    ...
    << "              -Wb,export_include=../" << node.name_ << "_stub_export.h \\" << std::endl
    << std::endl
    ...
    << std::endl
    << "    " << node.name_ << "_DDS.idl" << std::endl
    << "    ./ddstypes/" << node.name_ << "_DDSDcps.idl" << std::endl
    << "  }" << std::endl
    << "}" << std::endl
    << std::endl;
}
...
generate_stub_project (const CUTS_BE_IDL_Node & node)
{
  ...
  // Generate the project.
  this->ctx_.project_
    << "project (" << stub_name << ") : cuts_chaos_ccm_stub, avoids_ace_for_tao, cuts_codegen_defaults {" << std::endl
  ...
  this->ctx_.project_
    ...
    << "    TCPIP_" << node.name_ << "C.cpp" << std::endl
  ...
  this->ctx_.project_
    ...
    << "    TCPIP_" << node.name_ << "C.h" << std::endl
...
generate_skel_project (const CUTS_BE_IDL_Node & node)
{
  ...
  // Generate the project.
  this->ctx_.project_
    << "project (" << skel_name << ") : cuts_chaos_ccm_skel, avoids_ace_for_tao, cuts_codegen_defaults {" << std::endl
...
// generate_listing
//
void CUTS_BE_Project_Write_T <CUTS_CHAOS::Context, CUTS_BE_IDL_Node>::
...
// CUTS_BE_Project_File_End_T
//
bool CUTS_BE_Project_File_End_T <CUTS_CHAOS::Context, CUTS_BE_IDL_Node>::
generate (const CUTS_BE_IDL_Node & node)
{
  this->ctx_.project_
...
CUTS_CHAOS::Context, CUTS_BE_IDL_Node>::generate (void)
{
  if (this->ctx_.project_.is_open ())
    this->ctx_.project_.close ();
}
