...
template <typename P, typename T>
bool contains_t <PRED>::
operator () (const P & parent, const std::string & metatype, T & element)
{
  // Get the children of the parent.
  typedef GME::Collection_T <T> T_set;
  T_set children;

  T_set::size_type count = parent.children (metatype, children);
...
bool contains_t <T>::
find_i (ITER iter_begin, ITER iter_end,
        typename std::iterator_traits <ITER>::value_type & element)
{
  ITER iter = std::find_if (iter_begin, iter_end, this->predicate_);
...
//
template <typename P, typename T, typename PRED>
bool create_if (P & parent, const std::string & metatype,
                T & element, PRED predicate)
{
  // Determine if the parent has an element that matches the specified
  // predicate. If it does contain such an element then we need to
  // create one for the caller.
  if (!predicate (parent, metatype, element))
    return false;

  element = T::_create (metatype, parent);
  return true;
}
...
//
template <typename P, typename T, typename PRED>
bool create_if_not (P & parent, const std::string & metatype
                    T & element, PRED predicate)
{
  // Determine if the parent has an element that matches the specified
  // predicate. If it does not contain such an element then we need to
