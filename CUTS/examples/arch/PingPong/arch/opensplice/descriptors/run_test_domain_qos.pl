...
$status = 0;
$dat_file = "PingPong_DomainQoS.nodemap";
$cdp_file = "PingPong_DomainQoS.cdp";
...
sub run_node_daemons {
  for ($i = 0; $i < $daemons; ++$i)
  {
      ...
      $d_param = "-ORBEndpoint $iiop -s $node_app -n $nodename=$iorfile -t 30 --instance-nc corbaloc:rir:/NameService --locality-config $CUTS_ROOT/bin/handlers/opensplice.locality.config";
