...
for i in `ls ${CONFIG_LIST}`; do
  ...
  CONFIG_NUMBER=`echo $i | sed "s/birthtest-template-assembly//" | sed "s/.cdp//"`
  CONFIG_TEMPLATE=birthtest-template-assembly${CONFIG_NUMBER}.cdp
  ...
  $CUTS_ROOT/bin/cuts-template --template=${CONFIG_TEMPLATE} --config-list=birthtest-template-assembly.config -o ${CONFIG_OUTPUT_DIR} --use-env
