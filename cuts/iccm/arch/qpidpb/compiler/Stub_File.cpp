...
/**
 * @class List_Protobuf_Impl_Includes
 *
 * Visitor that is responsible for listing the include files for
 * a CORBA's corresponding protobuf event type.
 */
class List_Protobuf_Impl_Includes : public iCCM::Scope_Visitor
{
public:
  //
  // List_Protobuf_Impl_Includes
  //
  List_Protobuf_Impl_Includes (std::ofstream & hfile)
    : hfile_ (hfile)
  {
  ...
  virtual int visit_eventtype (AST_EventType * node)
  {
    ...
    ACE_CString pb_event;
    if (!be_global->get_wrapper_eventtype (node, pb_event))
      return 0;

    // Search for the type declaration of this DDS event type. It
    // should be an AST_Struct type.
    Identifier * id = 0;
    ACE_NEW_RETURN (id, Identifier (pb_event.c_str ()), -1);
    ...
    // calculate the Protobuf_impl.h include file.
    ...
    {
      this->hfile_ << include_t (basename + ".pb.h");
...
int Stub_File::visit_root (AST_Root * node)
{
  ...
  this->hfile_
    ...
    << include_t ("cuts/iccm/arch/qpidpb/servant/QpidPB_Traits_T.h")
    << std::endl;

  // List the include files for the DDS event types.
  List_Protobuf_Impl_Includes list_includes (this->hfile_);
